...
public class DIController {
	...
	public String myCal(Model model) {
		...
		/*
		 스프링컨테이너 생성 :xml파일을 파싱하여 파싱된 내용을 기반으로 ctx참조변수를 생성한다.
		 */
		AbstractApplicationContext ctx = new GenericXmlApplicationContext(configLocation);
		
		/*
		 XML설정파일에서 생성한 빈을 getBean()을 통해 주입받아 참조변수에 할당한다.
		 new 연산자를 통해 생성한 것과 동일하지만 외부설정파일에서 미리 생성한 것을 주입(injection)받아 사용하고 있다.
		 */
		CalculatorDTO myCal = ctx.getBean("myCal", CalculatorDTO.class);
		
		/*
		 주입받은 빈을 통해 사칙연산을 수행하고, 결과값을 model객체에 저장한 후 view로 전달한다.
		 */
	...
	public ModelAndView myAvengers() {
		...
		//아이언맨 빈을 주입받은 후 정보출력을 위해 setter를 호출한다.
		...
		//model객체에 정보저장과 뷰에 대한 설정을 동시에 처리
	...
	public ModelAndView myAnnotation() {
		...
		String str1 = "이름:"+mem1.getName() +"<br/>";
		 ...
		 str1 += "BMI결과:"+mem1.bmiCalculation()+"<br/>";
