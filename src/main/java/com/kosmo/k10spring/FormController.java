...
/*
 사용자의 요청을 제일먼저 받는 DispatcherServlet은 기본 패키지로 지정된
 com.kosmo.k10spring을 스캔하여 컨트롤러 클래스를 찾는다.
 ...
 @RequestMapping
 	...
 	나머지경로명으로 이루어진다. 요청명의 매핑정보로 메소드를 호출하게 되므로
 	메소드명은 큰 의미가 없다.
...
public class FormController {
	...
	/*
	 폼값받기1] 파라미터로 전달되는 값을 HttpServletRequest 객체를 통해 받는다.
	...
	public String loginMember(HttpServletRequest req, Model model) {
		...
		/*
		 View의 페이지경로를 반환한다. 아래처럼 String으로 반환하면 ViewResolver가 전체경로를
		 조립하여 웹브라우저에 출력한다.
		 */
	...
	/*
	 폼값받기2] @RequestParam 어노테이션으로 폼값받기
	 	파라미터 형식으로 아래와같이 사용한다. 
	 	이와같이 하면 해당 메소드 내에서 변수명을 그대로 사용할 수 있다.
	 */
	...
	/*
	 ...
	 조건1 : 쿼리스트링으로 전달되는 파라미터의 갯수와 폼값을 저장할 객체(DTO 혹은 VO)의 멤버변수의
	 	갯수가 동일할때 사용할 수 있다.
	 조건2 : 커맨드객체를 사용할때는 클래스명 앞글자를 소문자로 바꾼 형태의 매개변수를 사용해야 한다.
	  	EX) MemberDTO -> memberDTO
	 조건3 : 커맨드객체를 만들때 파라미터의 갯수는 달라도 상관없으나 파라미터명과 멤버변수명은 동일해야 되고
	...
	/*
	 폼값받기4] @pathVariable 어노테이션으로 폼값받기
	 	...
	 	파라미터를 마치 경로명처럼 사용하게 되고, 이때 주의할점은 웹브라우저는
	 	요청명을 경로로 인식하므로 리소스를 사용할때 경로를 주의해야 한다.
	 	또한 파라미터의 갯수가 틀려지면 404에러가 발생된다.
	 */
