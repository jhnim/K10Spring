...
public class TicketDAO {
	...
	/*
	 Spring-JDBD를 사용하기 위한 멤버변수와 setter()
	  트랜잭션 처리를 위한 메니져클래스의 멤버변수와 setter() 생성
	 */
	...
	public void buyTicket(final TicketDTO dto) {
		...
		/*
		 	...
		 	만약 하나라도 업무에 문제(예외)가 생기게 된다면 예외가 발생하여 모든 업무는 rollback되게 될것이다.
		 */
		try {
			//티켓 구매 금액에 대한 DB처리.
			...
			//티켓 구매 갯수에 대한 DB처리
			/*
			 check제약조건에 의해 구매갯수가 5장을 초과하는 경우 제약조건 위배로 예외가 발생한다.
			 */
			template.update(new PreparedStatementCreator() {
			...
			/*
			 티켓구매갯수가 5장 이하인 경우 정상처리되어 모든 작업이 commit된다.
			 */
			...
			e.printStackTrace();
			/*
			 티켓구매 갯수가 5장을 초과하는 경우에는 예외가 발생하여 모든 작업이 rollback된다.
			 */
			System.out.println("제약조건을 위배하여 카드결재와 티켓구매 모두가 취소되었습니다.");
